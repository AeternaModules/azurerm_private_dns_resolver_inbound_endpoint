...
output "private_dns_resolver_inbound_endpoints_ip_configurations" {
  ...
  value       = { for k, v in azurerm_private_dns_resolver_inbound_endpoint.private_dns_resolver_inbound_endpoints : k => one(v.ip_configurations) if v.ip_configurations != null && length(v.ip_configurations) > 0 }
}
